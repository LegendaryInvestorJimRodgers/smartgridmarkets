...
if (__name__ == '__main__'):
    ...
    for gamma in np.arange(0.001, 0.0044, 0.0008):
        temp = []
        try:
            counter, prices, belief = main(gamma)
            temp = np.append(temp, counter)
            print("for gamma:", gamma, " we see ", counter)
        except:
            print("for gamma:", gamma, " we see divergence")
            temp = np.append(temp, np.inf)
            break
        values.append(np.max(prices[1000:]) - np.min(prices[1000:]))
