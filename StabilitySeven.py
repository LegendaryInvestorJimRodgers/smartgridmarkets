...
def main(beta, r):
    ...
    return np.max(prices[3000:]) - np.min(prices[3000:]), prices, belief1, belief2, belief3, belief4, belief5
...
if (__name__ == '__main__'):
    ...
    for r in np.arange(0.001, 1, 0.1):
        ...
        for beta in np.arange(70, 90, 4):
            try:
                variances, prices, belief1, belief2, belief3, belief4, belief5 = main(beta, r)
                temp = np.append(temp, variances)
                print("for r:", r, " and beta:", beta, " we see ", variances)
            except:
                print("for r:", r, " and beta:", beta, " we see divergence")
                temp = np.append(temp, np.inf)
                continue
        values.append(temp)
        beliefMat1.append(np.mean(belief1))
        beliefMat2.append(np.mean(belief2))
        beliefMat3.append(np.mean(belief3))
        beliefMat4.append(np.mean(belief4))
        beliefMat5.append(np.mean(belief5))
    plt.imshow(values, interpolation='none', extent=[50, 80, 1, 0.001], aspect = 'auto')
    plt.xlabel('beta')
